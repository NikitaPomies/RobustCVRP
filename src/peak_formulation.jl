...
function build_BPF_model_test(I::Instance)

    # Create the model
    model = Model(CPLEX.Optimizer)
    
    @variable(model, x[i=1:n, j=1:n], Int)  # Binary variable: 1 if arc (i, j) is used
    @variable(model, p[i = 2:n], Bin)
    @variable(model, t[i = 2:n]>=0, Int)  
    @constraint(model,[i=1:n, j in 2:n], x[i,j] <=1)
    ...
    @objective(model, Min, 
    sum(I.distances[i,j]*x[i,j] for i in 1:n, j in 1:n))

    return model
end

model = build_BPF_model_test(instance)
