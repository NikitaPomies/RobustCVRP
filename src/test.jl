...
instance = read_instance("../data/n_5-euclidean_true")
...
function build_BPF_model(I::Instance)
    ...
    @variable(model, x0[j in customers], Int)  # On part du principe que x0 est binaire (souvent, il suffit de savoir si l'arc est utilisé).
    # Si nécessaire, on peut autoriser la valeur 2 en modifiant la borne supérieure :
    @constraint(model,[j in customers], x0[j]>=0)
    @constraint(model,[j in customers], x0[j]<=2)
    ...
    @variable(model, f[i in customers, j in customers], Int)
    @constraint(model,[i in customers, j in customers],f[i,j]>=0)
    ...
    @constraint(model, sum(x0[j] for j in customers) >= ceil(sum(I.demands)/I.capacity))
