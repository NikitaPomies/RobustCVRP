...
function solve_subproblem(x_bar)
    ...
    #fix.(sub_model[:x_copy], x_bar)
    optimize!(sub_model)
    @assert is_solved_and_feasible(sub_model) #removing dual here cause pb, pb n'est plus linéaire ? 
...
for i in 1:n
    for j in 1:n
        if (i != j)
            @constraint(model_d, mu_1[i, j] + lambda_1 >= (IT[i] + IT[j]) * x[i, j])
            @constraint(model_d, mu_2[i, j] + lambda_2 >= (IT[i] * IT[j]) * x[i, j])
        end
    end
end
